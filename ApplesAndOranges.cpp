...
they land at [4 + 2, 4 + 3, 4 + -4]. Oranges land at [12 + 3, 12 + -2, 12 + -4]. One apple and two oranges land in the inclusive range 7 - 10 so we print
...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    ...
    for (int j{0}; j < oranges.size(); j++) {
        if (oranges.at(j) < 0) {
            if ((b + oranges.at(j)) <= t && (b + oranges.at(j)) >= s) {
                num_oranges++;
            }
        }
    }

    cout << num_apples << endl << num_oranges << endl;
